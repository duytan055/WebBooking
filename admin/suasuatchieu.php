...
if (isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $sql = "SELECT * FROM suatchieu WHERE id_suat = $id";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (mysqli_num_rows($check_result) > 0) {
        ...
        $sql = "UPDATE suatchieu 
                SET id_phim = '$id_phim', 
                    id_phong = '$id_phong', 
                    date_chieu = '$date_chieu', 
                    thoi_gian = '$thoi_gian'
                WHERE id_suat = $id";
    ...
    <div class="container">
        ...
        <main class="main-content">
            <header class="box_search_bar">
                <h2>Sửa Suất Chiếu #<?php echo $suatchieu['id_suat']; ?></h2>
                <div class="user-info">
            ...
            <div class="form-container">
                <div class="info-box">
                    ...
                    <strong>Lưu ý:</strong> Hệ thống sẽ kiểm tra trùng lặp. Không thể có 2 suất chiếu cùng phòng, cùng ngày, cùng giờ.
                </div>
                ...
                <form method="POST">
                        ...
                        <select name="id_phim" required>
                            ...
                            <?php while ($row = mysqli_fetch_assoc($kq_phim)) { ?>
                                <option value="<?php echo $row['id_phim']; ?>"
                                    <?php echo $row['id_phim'] == $suatchieu['id_phim'] ? 'selected' : ''; ?>>
                        ...
                        <select name="id_phong" required>
                            ...
                            <?php while ($row = mysqli_fetch_assoc($kq_phong)) { ?>
                                <option value="<?php echo $row['id_phong']; ?>"
                                    <?php echo $row['id_phong'] == $suatchieu['id_phong'] ? 'selected' : ''; ?>>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" name="date_chieu" required
                            value="<?php echo $suatchieu['date_chieu']; ?>">
                    </div>

                    <div class="form-group">
                        <label>Giờ chiếu <span style="color: red;">*</span></label>
                        <input type="time" name="thoi_gian" required
                            value="<?php echo date('H:i', strtotime($suatchieu['thoi_gian'])); ?>">
